...
/* Creates page for detail of one sale */
...
class SalesDetPage extends Component {
   ...
   loadRecord() {
      Axios
         .get(serverUrl + "/api/exchange/S/" + this.state.urlProdId)
         .then  (
            res => {
               let newstate = {
...
/* ---------------------------------------------------------- */
   handleSave = (document_id, date, associate_id, internal_ref, external_ref,
                 currency, exchange_rate, notes) => {
      Axios
         .put(serverUrl + "/api/exchange/S/" + this.state.urlProdId,
         {
            document_id   : document_id,
            date          : date,
            associate_id  : associate_id,
   ...
   handleDelete = () => {
      Axios
         .delete(serverUrl + "/api/exchange/S/" + this.state.urlProdId)
         .then  (
            res => {
               let newstate = {msg1  : res.statusText}; 
               this.setState(newstate); 
